...
def make_dir():###今の時間が入った名前のディレクトリを作成
    global new_dir
    bagfiles_path = os.getcwd()##現在のディレクトリのパスをしゅと
    dt_now = datetime.datetime.now()##現在の時間を取得
    new_dir =str(bagfiles_path) + "/c_vcc" + str(dt_now.strftime('%m-%d-%H-%M'))+'/csv/'
...
def bag_to_csv():
    ...
    for i in range(kaisu):###回数分bagファイルを読み取り→cogposとcmd_velを並べる
        #cmd_cog="rostopic echo -b "+str(i)+".bag -p /wg/cogpos > " + str(new_dir) +str(i)+"_cog"+".csv"
        #cmd_vel="rostopic echo -b "+str(i)+".bag -p /wg/cmd_vel > " + str(new_dir) +str(i)+"_vel"+".csv"
...
def pandas_group():
    ...
    for i in range(kaisu):
        ...
        df_cog_time_m = (df_cog_time - df_cog.iloc[0,0]).astype(float) * 0.000000001
